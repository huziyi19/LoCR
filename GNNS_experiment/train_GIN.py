...
# GIN特定参数
NUM_LAYERS = 3       # GIN层数
TRAIN_EPS = False    # 是否学习可训练的epsilon
EPS = 0.0           # 初始epsilon值
...
class GINClassifier(nn.Module):
    """GIN图分类模型"""
    def __init__(self, input_dim, hidden_dim, num_classes, num_layers=3, train_eps=False, eps=0.0, pooling='mean'):
...
if __name__ == "__main__":
    ...
    print("\n模型、优化器、损失函数已初始化。")
    print(f"GIN配置: {NUM_LAYERS}层, 可训练epsilon={TRAIN_EPS}, 池化方式={POOLING}")
    print(model)
